...
function normalizeTitle(title) {
  return String(title || '').toLowerCase().replace(/[®™©]/g, '').replace(/[^a-z0-9]+/g, ' ').trim().replace(/\s+/g, ' ');
}
...
function extractGenres(product, localized) {
  const candidates = [...(product.Categories || []), ...(product.Properties?.Categories || []), ...(localized.Categories || [])];
...
(async () => {
  ...
  const leavingIds = new Set(lists.leaving);
  ...
  const removed = [...existingGames.values()].filter(game => game.id && !activeIds.has(game.id)).map(game => ({ ...game, status: 'removed', leavingSoon: false }));
  const merged = [...active, ...removed];
  const output = { updatedAt: new Date().toISOString(), source: 'Microsoft Xbox Game Pass catalog APIs + official ID@Xbox collection', market: MARKET, language: LANGUAGE, games: merged };

  fs.writeFileSync('games.json', JSON.stringify(output, null, 2) + '\n');
  console.log(`Official indie titles: ${officialIndieTitles.length}`);
  console.log(`Leaving soon: ${leavingIds.size}`);
